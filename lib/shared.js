...
class AWSAdapter {
  constructor (opts) {
    ...
    console.log('AWS Adapter started', opts)
  }
  getFilePath (room, tick) {
  ...
  write (room, tick, data, callback) {
    console.log('AWS WRITE', room, tick)
    const file = this.getFilePath(room, tick)
    return Promise.resolve(data)
      ...
      .tap(res => {
        console.log('AWS WRITE', room, tick, res)
      })
      .catch(err => console.error('AWS WRITE', room, tick, err.stack || err))
      .asCallback(callback)
  }
}
let adapters = {
